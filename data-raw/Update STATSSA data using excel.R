...
system("docker run -d -p 4448:4444 -p 7909:7900 -e SE_NODE_SESSION_TIMEOUT=57868143 --shm-size 2g selenium/standalone-chrome")
Sys.sleep(10)
con <- pipe("docker ps")
...
Data_to_include <- import_vars$file_id

Links_to_download <- STATSSA_links[grepl(paste(Data_to_include, collapse = "|"), STATSSA_links)]
...
downloaded_files <- downloaded_files[!grepl("discontinued", downloaded_files)]
...
for (i in seq_len(nrow(import_vars))) {

  data_xlsx <- unzip_and_read(import_vars$file_id[i],
                              import_vars$file_number[i],
                              import_vars$sheet_number[i])

    data_xlsx_long <- data_xlsx %>%
      pivot_longer(-(H01:import_vars$final_column[i]), names_to = "Date_original", values_to = "Value") %>%
      mutate(Link = str_replace(downloaded_files[grepl(import_vars$file_id[i], downloaded_files)],
    ...
    temp_name <- paste0("STATSSA_",import_vars$name[i])
